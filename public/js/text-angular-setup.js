...
.run(['taRegisterTool', '$window', 'taTranslations', 'taSelection', 'taToolFunctions', '$sanitize', 'taOptions', 'fileDialog', function(taRegisterTool, $window, taTranslations, taSelection, taToolFunctions, $sanitize, taOptions, fileDialog){
			...
			fileDialog.openFile(function(files){
				...
				imboClient.addImage(files[0], {
		            onComplete: function(err, imageIdentifier, res){
		            	loading.hide();
		            	if (err) {
                        return bootbox.alert("Xin lỗi, upload ảnh không thành công, file upload phải là ảnh có kích thước không quá 16MB, vui lòng kiểm tra và thử lại!");
                    }
	        	 		var imageUrl = imboClient.getImageUrl(imageIdentifier);
	            		imageUrl.maxSize({width:800}).compress({level:100});
	            		return ref.$editor().wrapSelection('insertImage', imageUrl.toString(), true);
		            },
